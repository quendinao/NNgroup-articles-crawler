...
class App(tk.Tk):
    ...
    def create_styles(self):
        ...
        self.style.configure('Title.TLabel', background=BG_COLOR, foreground=TEXT_COLOR, font=("Segoe UI Semibold", 16))
    ...
    def create_widgets(self):
        ...
        # Study Guide URL Input Row
        url_label = ttk.Label(inputs_card, text="Study Guide URL:", style='CardLabel.TLabel')
        url_label.grid(row=0, column=0, sticky="w", padx=(pad_x, 10), pady=(pad_y+5, pad_y))
        ...
        self.url_entry.grid(row=0, column=1, columnspan=2, sticky="ew", padx=(0, pad_x), pady=(pad_y+5, pad_y))
        ...
        # Cookies status row
        cookies_label = ttk.Label(inputs_card, text="Cookies:", style='CardLabel.TLabel')
        cookies_label.grid(row=2, column=0, sticky="w", padx=(pad_x, 10), pady=pad_y)
        ...
        import_cookies_btn.grid(row=2, column=2, padx=(10, pad_x), pady=pad_y)
    ...
    def update_cookies_status(self):
        ...
        if cookies_file:
            ...
            self.cookies_status_var.set("Not found (use 'Import Cookies' to add)")
    ...
    def import_cookies(self):
        ...
        if file_path:
            try:
                dest_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "youtube_cookies.txt")
                shutil.copy(file_path, dest_path)
                self.update_cookies_status()
                messagebox.showinfo("Success", "Cookies imported successfully!")
